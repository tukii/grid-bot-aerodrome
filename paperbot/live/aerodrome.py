...
class NonceManager:
    ...
    def update_w3(self, new_w3):
        """Update w3 reference after RPC switch (fix MEDIA: _switch_rpc)."""
        self.w3 = new_w3
...
class AerodromeLive:
    ...
    def get_nonce_manager(self, account) -> NonceManager:
        ...
        FIX (MEDIA): en la primera creación, sincroniza el nonce desde la
        chain y lo informa en log para facilitar diagnóstico.
        """
        ...
        if self._nonce_manager is None or self._nonce_manager_addr != addr:
            ...
            # Sincronizar y logear nonce al arrancar (diagnóstico)
            try:
                self._nonce_manager.sync()
                logger.info("NonceManager init — nonce detected for %s: %d",
                            addr, self._nonce_manager._nonce)
            except Exception as e:
                logger.warning("NonceManager init sync failed: %s", e)
